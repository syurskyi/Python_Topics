...
class MediumTestCase(unittest.TestCase):
    ...
    def test_medium_input(self):
        with self.assertRaises(TypeError):
            self.assertEquals(avg(1, 2, 3, "Mohammad"), 2)

    def test_medium_input_two(self):
        with self.assertRaises(TypeError):
            self.assertEquals(avg(10, 10, 10, 10, "10"), 10)
...
class HardTestCase(unittest.TestCase):
    ...
    def test_hard_input_three(self):
        with self.assertRaises(TypeError):
            self.assertEquals(avg(10, 10, 10, 10, frozenset), 10)

    def test_hard_input_four(self):
        with self.assertRaises(TypeError):
            self.assertEquals(avg(10, 10, 10, 10, set), 10)
