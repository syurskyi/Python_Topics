...
logger = l____.getLogger('test_logger')
...
l____.basicConfig(format='%(asctime)s %(levelname)s:%(message)s', level=l____.DEBUG)
...
l____.basicConfig(format='%(asctime)s %(levelname)-8s [%(filename)s:%(lineno)d] %(message)s',
    datefmt='%d-%m-%Y:%H:%M:%S',
    level=l____.DEBUG)

logger = l____.getLogger('my_app')
...
another_logger = l____.getLogger('my_app.database')  # gets a child logger called 'database' of 'my_app'
