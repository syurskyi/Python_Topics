...
def file_lister():
    ...
    node_classes _ ['Read', 'ReadGeo', 'Camera']
    node_list _ []
    ...
    ___ node __ node_list:
        filepath _ node['file'].value()
        filename _ os.path.basename(filepath)
        
        # Separate the file name and version number
        filename_no_version _ filename[0:filename.find('_v')]
        version_number _ filename[filename.find('_v')+1:filename.find('_v')+6]
