...
def parse_advanced_settings(*args):
    ...
    try:
        with open(advanced_settings_file, 'r') as advset:
            advanced_settings_lines = [ line ___ line __ advset.readlines() if line[:le.(vars_line)] == vars_line ]
    except:
        pass
...
def find_looks_group(*args):
    try:
        return [ node ___ node __ ?.allNodes('Group') if node.knob('looks_group') ][0]
    except IndexError:
...
def get_connected_nodes(node, *args):
    ...
    ___ node __ ?.selectedNodes():
        if node.Class() not __ ignore_list:
            connected_nodes.ap..(node)
...
def get_snapshot_dirs(*args):
    ...
    return [ d ___ d __ os.listdir(snapshot_root) if os.path.isdir(os.path.join(snapshot_root, d)) ]
