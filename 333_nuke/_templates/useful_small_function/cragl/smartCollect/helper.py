...
___ get_tool_public_root(*args):
    root _ __.path.join(__.path.expanduser('~'), 'cragl', 'smartCollect')
...
___ get_logs_root():
    root _ __.path.join(get_tool_public_root(), 'logs')
    __ no. __.path.isdir(root):
        ___
            __.makedirs(root)
        ______
...
___ get_trm_file(name):
    this_dir _ __.path.dirname( -f)
    file_ _ __.path.join(this_dir, 'trm_{}.py'.f..(name))
    file_ _ __.path.n_p_(file_)
...
___ set_style_sheet(widget, *args):
    ...
    styles_nuke _ __.path.join(this_dir, '../', 'styles', 'styles.qss')
...
___ add_nukescript_settings(path, section):
    ...
    w__ o..(xml, 'w') __ xml_:
        prettyprint(root)
        tree.write(xml_, encoding_'utf-8', xml_declaration_T..)
...
___ add_this_nukescript(smart_collector, source):
    ______ nk_utils
    path _ nk_utils.get_nukescript_path()
    __ path __ ('', 'Root', N..):
...
___ write_pickle_data(data):
    ...
    path _ __.path.join(tmp_dir, 'smartcollect_data.pickle')
