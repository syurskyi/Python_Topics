...
def load_presets():
    ...
    if check_xml_ok(settings_xml):
        settings_tree = ET.parse(settings_xml)
        settings_root = settings_tree.getroot()
        for preset in settings_root.find('backdrops').findall('backdrop'):
            presets[preset.get('name')] = {'icon': preset.get('icon'),
             'color': [ float(val.strip()) for val in preset.get('color').split(',') ]}
...
def update_preset(preset_name, key, value):
    xml, root, tree = get_xml_elements()
    for preset in root.find('backdrops').findall('backdrop'):
...
def get_next_link_name():
    ...
    pattern = '(link_)(\\d+)'
    node_names = sorted([ node.name() for node in nuke.allNodes() if re.search(pattern, node.name()) ], key=split_nr)
    if node_names:
...
def zoom(node):
    nuke.zoom(1.0, (int(node.xpos()), int(node.ypos())))


def get_repr_class_nodes():
    nodes = []
    for node_class in ['PostageStamp', 'Dot']:
        nodes.extend(nuke.allNodes(node_class))
...
def list_from_string(string):
    if isinstance(string, list):
        return string
    if ',' in string:
        return [ cls.strip() for cls in string.split(',') ]
    return [string]
...
def get_root_favorites_knob():
    favorites_knob = nuke.root().knob(PREFIX_FAVORITES)
    if favorites_knob:
...
def add_smartlink_tab_widgets():
    root = nuke.root()
    if SMARTLINK in root.knobs():
        return
    tab = nuke.Tab_Knob(SMARTLINK)
    favorites = nuke.String_Knob(PREFIX_FAVORITES, FAVORITES)
...
def add_to_root_favorites(uid):
    ...
    favorites = [ fav.strip() for fav in favorites_knob.getValue().split(',') if fav ]
    if uid in favorites:
        return
    favorites.append(uid)
    favorites_knob.setValue(', '.join(favorites))
...
def swap_presets(preset1, preset2):
    ...
    xml, root, tree = get_xml_elements()
    presets = []
    presets.append(_find_preset(root, preset1))
    presets.append(_find_preset(root, preset2))
    if not all(presets):
...
def add_preset(name, color, icon):
    ...
    elem.set('icon', icon)
    root.find('backdrops').append(elem)
    with open(xml, 'w') as xml:
        prettyprint(root)
        tree.write(xml, encoding='utf-8', xml_declaration=True)
