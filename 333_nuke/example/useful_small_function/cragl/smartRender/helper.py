...
def get_next_renderjob(*args):
    ...
    for job in jobs_root.find('jobs').findall('job'):
        for setting in job.findall('setting'):
...
def get_all_write_nodes_data(*args):
    write_data = {}
    all_write_nodes = [ node for node in nuke.allNodes('Write') if node['disable'].getValue() == 0.0 ]
...
def calculate_process_precentage(job_id, frame, *args):
    ...
    for job in jobs_root.find('jobs').findall('job'):
        if job.get('id') == job_id:
            for data in job.find('process').findall('data'):
            ...
            for f in job.find('frames').findall('frame'):
                if f.text == str(frame):
...
def get_job_data(job_id, *args):
    ...
    for job in jobs_root.find('jobs').findall('job'):
        if job.get('id') == job_id:
            ...
            for frame in job.find('frames').findall('frame'):
                frames_to_process.append(int(frame.text))
...
def insert_as_read_node(job_details, write = None, *args):
    if write:
        ...
        read['file'].setValue(write['file'].getValue())
        ...
        nuke.zoom(1.0, (write.xpos(), write.ypos()))
        nukescripts.connect_selected_to_viewer(0)
    else:
        read = nuke.createNode('Read')
        read['file'].setValue(job_details['render_path'])
        read['first'].setValue(int(job_details['render_start']))
        read['origfirst'].setValue(int(job_details['render_start']))
        read['last'].setValue(int(job_details['render_end']))
        read['origlast'].setValue(int(job_details['render_end']))
        read['on_error'].setValue('nearest frame')
        read.selectOnly()
        nuke.zoom(1.0, (read.xpos(), read.ypos()))
        nukescripts.connect_selected_to_viewer(0)
...
def force_create_render_dir(*args):
    ...
    os_dir = nuke.callbacks.filenameFilter(dir_name)
    try:
        os.makedirs(os_dir)
    except OSError as e:
...
def check_settings_xml_values_exist():
    ...
    for key, value in settings_current.items():
...
def load_sounds(*args):
    ...
    if not os.path.isdir(sounds_dir):
        return
    for file_ in os.listdir(sounds_dir):
        if os.path.splitext(file_)[1] in ('.wav', '.WAV'):
            sounds.append(file_)
...
def load_presets(*args):
    ...
    for presets in settingsroot.find('presets'):
        preset_name = presets.get('name')
        preset = {}
        for setting in presets.findall('setting'):
            if setting.get('name') == 'range':
                ...
                if not preset['thread_count']:
                    preset['thread_count'] = nuke.env['numCPUs'] / 2
...
def load_settings(*args):
    ...
    if check_xml_ok(settings_xml):
        ...
        for path in settingsroot.find('cache').findall('path'):
            cache_paths.append({'mode': path.get('mode'),
             'path': path.text})
...
def get_caches_by_mode(mode, *args):
    return [ cache['path'] for cache in load_settings()['cache'] if cache['mode'] == mode ]
...
def check_xml_value_exists_current(section, key1, value1, text, key2 = '', value2 = ''):
    ...
    item_found = 0
    current_found = False
    for child in root.find('presets').findall('preset'):
...
def delete_cache_path(path, *args):
    ...
    settings_tree = ET.parse(settings_xml)
    settings_root = settings_tree.getroot()
    for path_element in settings_root.find('cache').findall('path'):
...
def get_all_views(range_ = 10):
    try:
        ...
        for i in range(range_):
            nuke.activeViewer().previousView()

        for i in range(range_):
            if nuke.activeViewer().view() not in views:
                views.append(nuke.activeViewer().view())
            nuke.activeViewer().nextView()
