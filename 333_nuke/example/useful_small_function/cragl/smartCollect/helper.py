...
def load_nukescripts_settings(section):
    ...
    nukescripts = []
    for nukescript in root.find(section).findall('nukescript'):
        nukescripts.append(nukescript.text)
...
def load_nukescripts_progress(path):
    ...
    for nukescript in root.find('archive').findall('nukescript'):
...
def add_this_nukescript(smart_collector, source):
    ...
    if path in ('', 'Root', None):
        ...
        return
    elif path in smart_collector.table_nukescripts.nukescripts:
...
def get_selected_widgets(table):
    selected_elements = []
    for row in table.selectedIndexes():
        selected_elements.append(table.cellWidget(row.row(), 0))
