...
Through examples, hopefully you’ll end this video with a good understanding of what regular expressions are for,
how to use them, and how to learn more when you get stuck (because you will, undoubtedly!).

The first thing to understand is that regular expressions are a language. Not a programming language,
...
I notice another pattern, which is `a1 a3 a9 a5 a4` (that’s the entire string, followed by nothing).
This will be the case for every example.
...
I’ve given you a few examples of patterns,
but really it requires time and analysing many patterns to be able to get quicker at this.

Now let’s look at the regex syntax. I won’t cover absolutely everything you can learn about regex
(that would take an entire course!), but the main components.
...
`Jose@tecladocode\.com` would also match the entire pattern,
since it is the pattern. Notice the `\.` so that the `.` doesn’t mean “anything”. With the backslash in front,
it matches the actual `.` character.

Next, we have *character sets*. This is a character set: `[abc]`. For this string:

`charlie`, `[abc]` would match the `c` and the `a`. For `Charlie`
though it would only match the `a`—these are case sensitive.
...
There’s a lot more to regular expressions, but this will certainly get you started.
http://regexr.com has more information, cheatsheets, and also the online editor for you to try things out.

If you have any problems with any regex expressions,
feel free to post in the Slack channel for help (even if it’s unrelated to the course). We’ll be glad to help you out!
"""