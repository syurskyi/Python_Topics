...
class Cipher(object
    ...
    ___ __init__(self, key=None
        ...
        __ not key:
            key = ''.join([choice(ascii_lowercase) ___ _ in range(100)])
    ...
    ___ encode(self, clear_text
        ...
        key_func = self._inf_iter([ord(c)-97 ___ c in self.key])
    ...
    ___ _shift(text, key_func
        ...
        text = [c ___ c in text.lower() __ c.isalpha()]
        shift_dist = lambda x, s: chr((ord(x)-97+s) % 26 + 97)
        r_ ''.join([shift_dist(c, k) ___ c, k in zip(text, key_func)])
