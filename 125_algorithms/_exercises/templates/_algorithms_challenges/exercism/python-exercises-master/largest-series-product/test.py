...
from largest_series_product ______ largest_product


# test cases adapted from `x-common//canonical-data.json` @ version: 1.0.0

class SeriesTest(unittest.TestCase
    ___ test_finds_the_largest_product_if_span_equals_length(self
        self.assertEqual(largest_product("29", 2), 18)

    ___ test_can_find_the_largest_product_of_2_with_numbers_in_order(self
        self.assertEqual(largest_product("0123456789", 2), 72)

    ___ test_can_find_the_largest_product_of_2(self
        self.assertEqual(largest_product("576802143", 2), 48)

    ___ test_can_find_the_largest_product_of_3_with_numbers_in_order(self
        self.assertEqual(largest_product("0123456789", 3), 504)

    ___ test_can_find_the_largest_product_of_3(self
        self.assertEqual(largest_product("1027839564", 3), 270)

    ___ test_can_find_the_largest_product_of_5_with_numbers_in_order(self
        self.assertEqual(largest_product("0123456789", 5), 15120)

    ___ test_can_get_the_largest_product_of_a_big_number(self
        self.assertEqual(
            largest_product(
                "73167176531330624919225119674426574742355349194934", 6),
            23520)

    ___ test_reports_zero_if_the_only_digits_are_zero(self
        self.assertEqual(largest_product("0000", 2), 0)

    ___ test_reports_zero_if_all_spans_include_zero(self
        self.assertEqual(largest_product("99099", 3), 0)

    ___ test_rejects_span_longer_than_string_length(self
        with self.assertRaises(ValueError
            largest_product("123", 4)

    ___ test_reports_1_for_empty_string_and_empty_product_0_span(self
        self.assertEqual(largest_product("", 0), 1)

    ___ test_reports_1_for_nonempty_string_and_empty_product_0_span(self
        self.assertEqual(largest_product("123", 0), 1)

    ___ test_rejects_empty_string_and_nonzero_span(self
        with self.assertRaises(ValueError
            largest_product("", 1)

    ___ test_rejects_invalid_character_in_digits(self
        with self.assertRaises(ValueError
            largest_product("1234a5", 2)

    ___ test_rejects_negative_span(self
        with self.assertRaises(ValueError
            largest_product("12345", -1)

    @unittest.skip("extra-credit")
    ___ test_project_euler_big_number(self
        series = (
