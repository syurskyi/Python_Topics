...
class ScaleGeneratorTest(unittest.TestCase
    ...
    ___ test_another_chromatic_scale(self
    ...
    ___ test_naming_major_scale(self
        major = Scale('G', 'major', 'MMmMMMm')
        expected = 'G major'
        actual = major.name
        self.assertEqual(expected, actual)

    ___ test_major_scale(self
        major = Scale('C', 'major', 'MMmMMMm')
        expected = ['C', 'D', 'E', 'F', 'G', 'A', 'B']
        actual = major.pitches
        self.assertEqual(expected, actual)

    ___ test_another_major_scale(self
        major = Scale('G', 'major', 'MMmMMMm')
        expected = ['G', 'A', 'B', 'C', 'D', 'E', 'F#']
        actual = major.pitches
        self.assertEqual(expected, actual)

    ___ test_minor_scale(self
        minor = Scale('f#', 'minor', 'MmMMmMM')
        expected = ['F#', 'G#', 'A', 'B', 'C#', 'D', 'E']
        actual = minor.pitches
        self.assertEqual(expected, actual)

    ___ test_another_minor_scale(self
