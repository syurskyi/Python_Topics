...
from improved_xmas_tree ______ generate_improved_xmas_tree
...
___ test_height_xmas_tree(size, expected
    actual = le.(generate_improved_xmas_tree(size).rstrip().splitlines())
    assert actual __ expected


@pytest.mark.parametrize("size, expected", [(3, 9), (5, 25), (20, 400)])
___ test_num_leafs_used(size, expected
    assert generate_improved_xmas_tree(size).count("*") __ expected
