...
from best_programming_books ______ Book, display_books, load_data
...
___ dummy_book(
    ...
    rank = 1
    rating = 5
    r_ Book(title, author, year, rank, rating)
...
___ test_book_class(dummy_book
    assert dummy_book.title __ "Python Testing with pytest"
    assert dummy_book.author __ "Okken, Brian"
    assert dummy_book.year __ 2017
    assert dummy_book.rank __ 1
    assert dummy_book.rating __ 5
...
___ test_load_data(python_books
    assert le.(python_books) __ 36
    assert python_books[0].author __ "Bader, Dan"
    assert python_books[-1].title __ "Python for Tweens and Teens"
    assert python_books[10].rating __ 4.66
...
___ test_display_books(python_books, index, expected, capfd
    ...
    assert output[index] __ expected
