...
from guess ______ get_random_number, Game


@patch.object(random, 'randint')
___ test_get_random_number(m
    m.return_value = 17
    assert get_random_number() __ 17
...
___ test_guess(inp
    game = Game()
    # good
    assert game.guess() __ 11
    assert game.guess() __ 12
    # not a number
...
___ test_validate_guess(capfd
    ...
    assert out.rstrip() __ '1 is too low'

    assert not game._validate_guess(3)
    out, _ = capfd.readouterr()
    assert out.rstrip() __ '3 is too high'

    assert game._validate_guess(2)
    out, _ = capfd.readouterr()
    assert out.rstrip() __ '2 is correct!'
...
___ test_game_win(inp, capfd
    ...
    output = [line.strip() ___ line in out.split('\n') __ line.strip()]
