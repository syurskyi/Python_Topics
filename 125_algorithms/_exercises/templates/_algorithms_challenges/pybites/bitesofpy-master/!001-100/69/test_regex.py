from regex ______ (has_timestamp, is_integer,
...
___ test_remove_all_parenthesis_words(
    ...
    assert remove_all_parenthesis_words(input_string) __ expected
    input_string = 'ma__ (8.6) and science (9.1) where his strengths'
    expected = 'ma__ and science where his strengths'
    assert remove_all_parenthesis_words(input_string) __ expected
...
___ test_split_string_on_punctuation(
    ...
    assert split_string_on_punctuation(input_string) __ expected
...
___ test_remove_duplicate_spacing(
    ...
    assert remove_duplicate_spacing(input_string) __ expected
...
___ test_convert_emea_date_to_amer_date(
    assert convert_emea_date_to_amer_date('31/03/2018') __ '03/31/2018'
    assert convert_emea_date_to_amer_date('none') __ 'none'