...
___ _seek_next(grid: list, position: tuple
    ...
    __ row < le.(grid)-1 and grid[row + 1][col] __ next_val:
        r_ (row + 1, col), DOWN
    __ row > 0 and grid[row - 1][col] __ next_val:
        r_ (row - 1, col), UP
    __ col < le.(grid[row])-1 and grid[row][col + 1] __ next_val:
        r_ (row, col + 1), RIGHT
...
___ print_sequence_route(grid: str, start_coordinates=None
