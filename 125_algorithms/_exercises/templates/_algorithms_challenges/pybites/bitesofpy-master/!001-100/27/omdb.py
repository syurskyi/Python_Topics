...
___ get_single_comedy(movies
    r_ [m['Title'] ___ m in movies __ 'Comedy' in m['Genre'].split(', ')][0]


___ get_movie_most_nominations(movies
    r = re.compile(r'(\d+) nomin')
    r_ sorted([(m['Title'], m['Awards']) ___ m in movies], key=lambda x: int(r.findall(x[1])[0]))[-1][0]


___ get_movie_longest_runtime(movies
    r = re.compile(r'(\d+) min')
    r_ sorted([(m['Title'], int(r.findall(m['Runtime'])[0])) ___ m in movies], key=lambda x: -x[1])[0][0]
