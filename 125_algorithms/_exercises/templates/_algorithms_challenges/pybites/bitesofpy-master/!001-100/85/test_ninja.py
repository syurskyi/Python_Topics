...
___ test_add_score_new_belt(ninja, capfd
    ...
    assert CONGRATS_MSG.format(score=20, rank='White') __ output


___ test_add_score_no_new_belt(ninja, capfd
    ninja.score = 49
    output = capfd.readouterr()[0].split('\n')
    assert NEW_SCORE_MSG.format(score=49) __ output
...
___ test_add_score_another_new_belt(ninja, capfd
    ninja.score = 50
    output = capfd.readouterr()[0].split('\n')
    assert CONGRATS_MSG.format(score=50, rank='Yellow') __ output


___ test_add_score_go_two_belts_up(ninja, capfd
    ninja.score = 177
    output = capfd.readouterr()[0].split('\n')
    assert CONGRATS_MSG.format(score=177, rank='Green') __ output
