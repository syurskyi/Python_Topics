...
___ test_number_action_cards(deck, small_deck, big_deck
    assert sum(1 for card in deck __ card.action is not None) __ 8
    assert sum(1 for card in deck __ card.action is None) __ 24

    assert sum(1 for card in small_deck __ card.action is not None) __ 4
    assert sum(1 for card in small_deck __ card.action is None) __ 12

    assert sum(1 for card in big_deck __ card.action is not None) __ 16
    assert sum(1 for card in big_deck __ card.action is None) __ 48


___ test_all_action_cards_used(deck, small_deck, big_deck
    cards = [card.action for card in deck __ card.action is not None]
    assert sum(Counter(cards).values()) __ 8

    cards = [card.action for card in small_deck __ card.action is not None]
    assert sum(Counter(cards).values()) __ 4

    cards = [card.action for card in big_deck __ card.action is not None]
    assert sum(Counter(cards).values()) __ 16
...
___ test_deck_cards_numbers_constant(deck, small_deck, big_deck
    for i in list('ABCDEFGH'
        assert sum(1 for card in deck __ card.card[0] __ i) __ 4
    for i in list('ABCD'
        assert sum(1 for card in small_deck __ card.card[0] __ i) __ 4
    for i in list('ABCDEFGHIJKLMNOP'
        assert sum(1 for card in big_deck __ card.card[0] __ i) __ 4


___ test_deck_numbers_used(
    for i in range(1, 27
        deck = list(create_paw_deck(i))
        assert sum(1 for card in deck __ int(card.card[1:]) __ 1) __ i
