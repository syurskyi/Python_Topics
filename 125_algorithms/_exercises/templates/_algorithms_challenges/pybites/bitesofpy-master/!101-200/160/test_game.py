...
from Previous.game ______ get_winner
...
___ test_ties(player
    assert get_winner(player, player) __ 'Tie'
...
___ test_snake_breathes_air(
    assert get_winner('Snake', 'Air') __ 'Snake'


___ test_snake_drinks_water(
    assert get_winner('Snake', 'Water') __ 'Snake'


___ test_human_plants_tree(
    assert get_winner('Human', 'Tree') __ 'Human'


___ test_human_tames_wolf(
    assert get_winner('Human', 'Wolf') __ 'Human'


___ test_human_cleans_with_sponge(
    assert get_winner('Human', 'Sponge') __ 'Human'


___ test_human_writes_paper(
    assert get_winner('Human', 'Paper') __ 'Human'


___ test_human_breathes_air(
    assert get_winner('Human', 'Air') __ 'Human'


___ test_human_drinks_water(
    assert get_winner('Human', 'Water') __ 'Human'


___ test_human_slays_dragon(
    assert get_winner('Human', 'Dragon') __ 'Human'


@pytest.mark.parametrize("player", ['Wolf', 'Dragon'])
___ test_tree_shelters_wolf_and_dragon(player
    assert get_winner('Tree', player) __ 'Tree'


___ test_tree_outlives_sponge(
    assert get_winner('Tree', 'Sponge') __ 'Tree'


___ test_tree_becomes_paper(
    assert get_winner('Tree', 'Paper') __ 'Tree'


___ test_tree_produces_air(
    assert get_winner('Tree', 'Air') __ 'Tree'


___ test_tree_drinks_water(
    assert get_winner('Tree', 'Water') __ 'Tree'


___ test_tree_imprisons_devil(
    assert get_winner('Tree', 'Devil') __ 'Tree'


@pytest.mark.parametrize("player", ['Sponge', 'Paper'])
___ test_wolf_chews_up_sponge_and_paper(player
    assert get_winner('Wolf', player) __ 'Wolf'


___ test_wolf_breathes_air(
    assert get_winner('Wolf', 'Air') __ 'Wolf'


___ test_wolf_drinks_water(
    assert get_winner('Wolf', 'Water') __ 'Wolf'
...
___ test_wolf_bites_devils_heiny(
    assert get_winner('Wolf', 'Devil') __ 'Wolf'


___ test_sponge_soaks_paper(
    assert get_winner('Sponge', 'Paper') __ 'Sponge'


___ test_sponge_uses_air_pockets(
    assert get_winner('Sponge', 'Air') __ 'Sponge'


___ test_sponge_absorbs_water(
    assert get_winner('Sponge', 'Water') __ 'Sponge'


@pytest.mark.parametrize("player", ['Devil', 'Dragon'])
___ test_sponge_cleanses_devil_and_dragon(player
    assert get_winner('Sponge', player) __ 'Sponge'


___ test_sponge_cleans_guns(
    assert get_winner('Sponge', 'Gun') __ 'Sponge'


___ test_sponge_conducts_lightning(
    assert get_winner('Sponge', 'Lightning') __ 'Sponge'


___ test_paper_fans_air(
    assert get_winner('Paper', 'Air') __ 'Paper'


___ test_paper_covers_rock(
    assert get_winner('Paper', 'Rock') __ 'Paper'


___ test_paper_floats_on_water(
    assert get_winner('Paper', 'Water') __ 'Paper'


@pytest.mark.parametrize("player", ['Devil', 'Dragon'])
___ test_paper_rebukes_devil(player
    assert get_winner('Paper', player) __ 'Paper'


___ test_paper_outlaws_gun(
    assert get_winner('Paper', 'Gun') __ 'Paper'


___ test_paper_defines_lightning(
    assert get_winner('Paper', 'Lightning') __ 'Paper'


___ test_air_blows_out_fire(
    assert get_winner('Air', 'Fire') __ 'Air'


___ test_air_erodes_rock(
    assert get_winner('Air', 'Rock') __ 'Air'


___ test_air_evaporates_water(
    assert get_winner('Air', 'Water') __ 'Air'


___ test_air_chokes_devil(
    assert get_winner('Air', 'Devil') __ 'Air'


___ test_air_tarnishes_gun(
    assert get_winner('Air', 'Gun') __ 'Air'


___ test_air_freezes_dragon(
    assert get_winner('Air', 'Dragon') __ 'Air'


___ test_air_creates_lightningj(
    assert get_winner('Air', 'Lightning') __ 'Air'


@pytest.mark.parametrize("player", ['Devil', 'Dragon'])
___ test_water_drowns_devil_and_dragon(player
    assert get_winner('Water', player) __ 'Water'


___ test_water_erodes_rock(
    assert get_winner('Water', 'Rock') __ 'Water'


___ test_water_puts_out_fire(
    assert get_winner('Water', 'Fire') __ 'Water'
...
___ test_water_conducts_lightning(
    assert get_winner('Water', 'Lightning') __ 'Water'


___ test_dragon_commands_devil(
    assert get_winner('Dragon', 'Devil') __ 'Dragon'


@pytest.mark.parametrize("player", ['Lightning', 'Fire'])
___ test_dragon_breathes_lightning_and_fire(player
    assert get_winner('Dragon', player) __ 'Dragon'


___ test_dragon_rests_on_rock(
    assert get_winner('Dragon', 'Rock') __ 'Dragon'
...
___ test_dragon_spawns_snake(
    assert get_winner('Dragon', 'Snake') __ 'Dragon'


___ test_devil_hurls_rocks(
    assert get_winner('Devil', 'Rock') __ 'Devil'


___ test_devil_breaths_fire(
    assert get_winner('Devil', 'Fire') __ 'Devil'


@pytest.mark.parametrize("player", ['Scissors', 'Gun'])
___ test_devil_is_immune_to_scissors_and_gun(player
    assert get_winner('Devil', player) __ 'Devil'


___ test_devil_casts_lightning(
    assert get_winner('Devil', 'Lightning') __ 'Devil'


___ test_devil_eats_snake(
    assert get_winner('Devil', 'Snake') __ 'Devil'


___ test_devil_possesses_human(
    assert get_winner('Devil', 'Human') __ 'Devil'


@pytest.mark.parametrize("player", ['Gun', 'Scissors'])
___ test_lightning_melts_gun_and_scissors(player
    assert get_winner('Lightning', player) __ 'Lightning'
...
___ test_lightning_starts_fire(
    assert get_winner('Lightning', 'Fire') __ 'Lightning'
...
___ test_gun_targets_rock_tree_and_fire(player
    assert get_winner('Gun', player) __ 'Gun'


___ test_gun_outclasses_scissors(
    assert get_winner('Gun', 'Scissors') __ 'Gun'
