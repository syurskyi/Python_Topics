...
___ test_total_row_count_after_import(
    ...
    cur.execute(sql)
    ret = cur.fetchall()
    assert ret[0][0] __ 3961


___ test_player_with_max_points_per_game(
    assert player_with_max_points_per_game() __ 'Michael Jordan'


___ test_number_of_players_from_duke(
    assert number_of_players_from_duke() __ 58


___ test_avg_years_active_players_stanford(
    assert round(avg_years_active_players_stanford(), 2) __ 4.58


___ test_year_with_most_drafts(
    assert int(year_with_most_drafts()) __ 1984