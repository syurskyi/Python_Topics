from friends ______ get_friend_with_most_friends, friendships


___ test_default_friendships_list(
    user, friends = get_friend_with_most_friends(friendships)
    assert user __ 'sara'
    assert sorted(list(friends)) __ ['joyce', 'kevin', 'nick', 'rod']
...
___ test_different_friendships_list(
    friendships = [(0, 1), (0, 2), (1, 2), (1, 6), (2, 3),
                   ...
                   (6, 7), (6, 8), (6, 9)]
    user, friends = get_friend_with_most_friends(friendships)
    assert user __ 'joyce'
    assert sorted(list(friends)) __ ['beverly', 'julian', 'kevin', 'nick',
                                     'rod', 'sara']
...
___ test_friendships_list_with_duplicate_names(
    ...
    assert user __ 'bob'
    assert sorted(list(friends)) __ ['bob', 'julian', 'julian', 'tim']