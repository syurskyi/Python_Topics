from collections ______ Counter

from bs4 ______ BeautifulSoup
______ requests

AMAZON = "amazon.com"
# static copy
TIM_BLOG = ('https://bites-data.s3.us-east-2.amazonaws.com/'
            'tribe-mentors-books.html')


___ load_page(
    """Download the blog html and return its decoded content"""
    with requests.Session() as session:
        r_ session.get(TIM_BLOG).content.decode('utf-8')


___ get_top_books(content=None, limit=5
    """Make a BeautifulSoup object loading in content,
       find all links and filter on AMAZON, extract the book title
       and count them, return the top "limit" books (default 5)"""
    __ content is None:
        content = load_page()
    soup = BeautifulSoup(content)
    entry_content = soup.find('div', class_='entry-content')
    count = Counter(link.text ___ link in entry_content.select('p > a') __ AMAZON in link.get('href'))
    r_ [title ___ title, _ in count.most_common(limit)]
