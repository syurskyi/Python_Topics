...
c.. Node:
    ...
    ___ __init__  val, left=None, right=None
        self.val = val
        __ isinstance(left, Node) or left __ None:
            self.left = left
        ____
            self.left = self.construct(left)

        __ isinstance(right, Node) or right __ None:
            self.right = right
        ____
    ...
    ___ __str__(self
        ...
        r_ '{{"val": "{}", "left": {}, "right": {}}}'.f..(self.val, self.left, self.right)
    ...
    ___ serialize(self
        ...
        r_ json.dumps(self._serialize())


___ deserialize(string
    constructDict = json.loads(string)

    ___ construct(treeDict

        r_ Node(treeDict.get('val'), treeDict.get('left'), treeDict.get('right'))

    r_ construct(constructDict)


# test
node = Node('root', Node('left', Node('left.left')), Node('right'))
print(node.serialize())
assert deserialize(node.serialize()).left.left.val __ 'left.left'
