...
public:
    vector<int> topKFrequent(vector<int>& nums, int k) {
        ...
        ___(const auto &n: nums){
            num_count[n] += 1;
        }

        priority_queue<pair<int, int>, vector<pair<int, int>>, bigger> frequent_heap;
        // Build the min-heap with size k.
        ___(auto it = num_count.begin(); it != num_count.end(); it++){
            __(frequent_heap.size() < k){
                frequent_heap.push(*it);
            }
            else __(it->second >= frequent_heap.top().second){
                frequent_heap.pop();
                frequent_heap.push(*it);
            }
        }
...
public:
    vector<int> topKFrequent(vector<int>& nums, int k) {
        ...
        ___ (auto &i : m) heap.push_back({i.second, i.first});
