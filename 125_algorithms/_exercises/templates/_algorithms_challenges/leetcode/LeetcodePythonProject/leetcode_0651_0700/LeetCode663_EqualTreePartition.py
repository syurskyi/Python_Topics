...
class Solution(object
    ...
    ___ helper(self, root, sumVal, firstLevel
        __ not root:
            r_ False
        __ sumVal __ self.su.(root) and not firstLevel:
            r_ True
        __ (root.left and self.helper(root.left, sumVal+root.val+self.su.(root.right), False)) or\
            (root.right and self.helper(root.right, sumVal+root.val+self.su.(root.left), False)):
            r_ True
        r_ False
    ...
    ___ su.(self, root
        __ not root: r_ 0
        res = root.val
        res += self.su.(root.left)
        res += self.su.(root.right)
        r_ res
