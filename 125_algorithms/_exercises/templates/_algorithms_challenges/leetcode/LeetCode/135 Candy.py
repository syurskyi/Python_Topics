...
class Solution:
    ...
    ___ re_adjust(self, ratings, dp, ind
        # backward adjust
        original = dp[ind]
        __ original__1: r_  # no adjustment needed
        i = ind
        candy = 1
        w___ i>0 and ratings[i-1]>ratings[i]:  # test case [8, 7, 6, 5, 4, 3, 2, 1]
            dp[i] = candy
            candy += 1
            i -= 1
        # peak
...
__ __name____"__main__":
    ...
    assert Solution().candy([4, 2, 3, 4, 1])__9
    assert Solution().candy([1, 4, 3, 2, 1])__11
    assert Solution().candy([8, 7, 6, 5, 4, 3, 2, 1])__36
