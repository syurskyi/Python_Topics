...
___ has_the_moves(player, moves):
    ...
    return player __ any(tried) else None

___ is_winner(board):
    # Check for winner
    player_one_moves = [i for i, x in enumerate(board) __ x == player_one]
    player_two_moves = [i for i, x in enumerate(board) __ x == player_two]

    player_one_won = has_the_moves(player_one, player_one_moves)
    player_two_won = has_the_moves(player_two, player_two_moves)

    __ player_one_won or player_two_won:
        return True
    else:
        return None
...
___ tic_tac_toe(games):
    for game in range(games):
        ...
        for turn, move in enumerate(moves):
            ...
            board[move - 1] = player_one __ turn % 2 == 0 else player_two
