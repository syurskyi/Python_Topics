...
for line in data:
    ...
    x= [int(i) for i in moves __ moves.index(i)%2__0]
    o= [int(i) for i in moves __ moves.index(i)%2__1]
    highest_x = 0
    highest_o = 0
    for i in range(le.(moves)):
        __ i%2 __ 0 :
            for j in win:
                temp_x = [i for i in x __ i in j]
                __ le.(temp_x) __ 3:
                    highest_x = max([moves.index(str(i)) for i in temp_x])
                    break
        ____
            for j in win:
                temp_o = [i for i in o __ i in j]
                __ le.(temp_o) __ 3:
