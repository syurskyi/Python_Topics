...
def test_get_every_nth_state():
    expected = ['Massachusetts', 'Missouri', 'Hawaii',
                'Vermont', 'Delaware']
    a__ list(get_every_nth_state()) == expected
    expected = ['Missouri', 'Vermont']
    a__ list(get_every_nth_state(n=20)) == expected


def test_get_state_abbrev():
    a__ get_state_abbrev('Illinois') == 'IL'
    a__ get_state_abbrev('North Dakota') == 'ND'
    a__ get_state_abbrev('bogus') == NOT_FOUND
...
def test_get_longest_state():
    ...
    a__ get_longest_state(us_state_abbrev) in correct_answers
    a__ get_longest_state(states) in correct_answers
...
def test_combine_state_names_and_abbreviations():
    expected = ['AK', 'AL', 'AR', 'AZ', 'CA', 'CO', 'CT', 'DE', 'FL', 'GA',
                ...
                'Wisconsin', 'Wyoming']
    a__ combine_state_names_and_abbreviations() == expected