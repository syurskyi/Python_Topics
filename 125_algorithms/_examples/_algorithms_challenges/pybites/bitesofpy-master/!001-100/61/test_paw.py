...
def test_deck_size(deck, small_deck, big_deck):
    a__ len(deck) == 32
    a__ len(small_deck) == 16
    a__ len(big_deck) == 64


def test_number_action_cards(deck, small_deck, big_deck):
    a__ sum(1 for card in deck if card.action is not None) == 8
    a__ sum(1 for card in deck if card.action is None) == 24

    a__ sum(1 for card in small_deck if card.action is not None) == 4
    a__ sum(1 for card in small_deck if card.action is None) == 12

    a__ sum(1 for card in big_deck if card.action is not None) == 16
    a__ sum(1 for card in big_deck if card.action is None) == 48


def test_all_action_cards_used(deck, small_deck, big_deck):
    cards = [card.action for card in deck if card.action is not None]
    a__ sum(Counter(cards).values()) == 8

    cards = [card.action for card in small_deck if card.action is not None]
    a__ sum(Counter(cards).values()) == 4

    cards = [card.action for card in big_deck if card.action is not None]
    a__ sum(Counter(cards).values()) == 16
...
def test_action_cards_in_different_positions(deck):
    ...
    a__ action_cards != action_cards2

    deck3 = list(create_paw_deck())
    action_cards3 = [card.card for card in deck3 if card.action is not None]
    a__ action_cards != action_cards2 != action_cards3


def test_deck_cards_numbers_constant(deck, small_deck, big_deck):
    for i in list('ABCDEFGH'):
        a__ sum(1 for card in deck if card.card[0] == i) == 4
    for i in list('ABCD'):
        a__ sum(1 for card in small_deck if card.card[0] == i) == 4
    for i in list('ABCDEFGHIJKLMNOP'):
        a__ sum(1 for card in big_deck if card.card[0] == i) == 4


def test_deck_numbers_used():
    for i in range(1, 27):
        deck = list(create_paw_deck(i))
        a__ sum(1 for card in deck if int(card.card[1:]) == 1) == i
