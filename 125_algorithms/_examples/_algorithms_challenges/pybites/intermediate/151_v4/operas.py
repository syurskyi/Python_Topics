...
def _was_alive(composer, date):
    return _get_date(composer.born) <= _get_date(date) < _get_date(composer.died)
...
def operas_both_at_premiere(guest, composer):
    ...
    """
    if composer not in composers or guest not in composers:
        raise ValueError
    ...
    return [opera.play for opera in operas
            if (composer == opera.author and _was_alive(gst, opera.date)
                and _was_alive(comp, opera.date))]
