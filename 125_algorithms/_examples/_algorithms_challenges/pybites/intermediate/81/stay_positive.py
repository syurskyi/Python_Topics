...
def filter_tweets_on_polarity(tweets, keep_positive=True):
      ...
      if keep_positive:
            positive = [tweet for tweet in tweets if tweet.p.. > 0]
            return positive
      else:
            negative = [tweet for tweet in tweets if tweet.p.. < 0]
            return negative
...
def order_tweets_by_polarity(tweets, positive_highest=True):
      ...
      if positive_highest:
            highest = sorted(tweets, key=lambda x: x.p.., reverse=True)
            return highest
      else:
            lowest = sorted(tweets, key=lambda x: x.p..)
            return lowest
