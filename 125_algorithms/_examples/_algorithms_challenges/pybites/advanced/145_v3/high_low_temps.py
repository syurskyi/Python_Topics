...
def remove_leap_days(df):
    ...
    indexed by date.
    '''
    df.drop(df.index[(df.Date.dt.month == 2) & (df.Date.dt.day == 29)],
            inplace=True)
...
def historical_records(df, element):
...
def high_low_record_breakers_for_2015():
    ...
    lows_15 = df_15[df_15.Element == 'TMIN']
    highs_15 = df_15[df_15.Element == 'TMAX']
    ...
    rec_low = rb_low[rb_low.Data_Value_15 == rb_low.Data_Value_15.min()].iloc[0]
    rec_high = rb_high[rb_high.Data_Value_15 == rb_high.Data_Value_15.max()].iloc[0]
